...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void splashInitMethod() async {
    ...
    Get.find<BasicController>().getBussinessSetting().then((value) {
      ...
      if (setting.getAppLinkAndAppVersion(Platform.isAndroid ? 'maintenance_mode_for_android_for_driver' : 'maintenance_mode_ios_for_driver') != null &&
          setting.getAppLinkAndAppVersion('user_android_maintenance_mode') == '1') {
        showDialog(
          ...
          int.parse(setting.getAppLinkAndAppVersion(Platform.isAndroid ? 'app_version_for_driver' : "app_version_ios_for_driver") ?? '0')) {
        if (Platform.isAndroid) {
          showDialog(
            ...
            builder: (context) => UpdateDialog(
              ...
              skip: ((setting.getAppLinkAndAppVersion('force_update_android_for_driver') != null && setting.getAppLinkAndAppVersion('force_update_ios_for_driver') == '0'))
                  ? true
                  : false,
            ),
          ).then((value) {
          ...
          showDialog(
            ...
            builder: (context) => UpdateDialog(
              remark: 'Please update app to the latest version',
              skip: ((setting.getAppLinkAndAppVersion('force_update_ios') != null && setting.getAppLinkAndAppVersion('force_update_ios') == '0')) ? true : false,
            ),
          ).then((value) {
            if (value) {
              initMethod();
    ...
    if (auth.isLoggedIn()) {
      auth.getUserProfileData().then((value) {
        if (value.isSuccess) {
          ...
          Navigator.of(context).pushAndRemoveUntil(getCustomRoute(child: const WelcomeScreen()), (route) => false);
        }
      });
    } else {
      Navigator.of(context).pushAndRemoveUntil(getCustomRoute(child: const WelcomeScreen()), (route) => false);
